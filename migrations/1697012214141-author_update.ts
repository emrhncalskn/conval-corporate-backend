...
export class AuthorUpdate1697012214141 implements MigrationInterface {
    name = 'AuthorUpdate1697012214141'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`pages\` ADD CONSTRAINT \`fk_pages_author_id\` FOREIGN KEY (\`author_id\`) REFERENCES \`users\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`pages\` DROP FOREIGN KEY \`fk_pages_author_id\``);
        await queryRunner.query(`ALTER TABLE \`menu\` DROP FOREIGN KEY \`fk_menu_belong\``);
        ...
        await queryRunner.query(`CREATE INDEX \`FK_permissions_functions\` ON \`permissions\` (\`func_id\`)`);
        await queryRunner.query(`CREATE INDEX \`FK_permissions_roles\` ON \`permissions\` (\`role_id\`)`);
    }
